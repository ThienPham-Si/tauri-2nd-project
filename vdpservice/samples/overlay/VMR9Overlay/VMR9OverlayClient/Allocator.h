...
public:
    ...
    void PlayOverlay(bool b) { m_playOverlay = b; }
    bool PlayOverlay() { return m_playOverlay; }

    void PlayDirectX(bool b) { m_playDirectX = b; }
    bool PlayDirectX() { return m_playDirectX; }
...
protected:
   ...
   virtual void OnStop() { }
   virtual void OnNextImage(void* pImage, int width, int height, int pitch) { }
   SIZE m_imageSize;
...
private:
    ...
    public:
       ...
       HRESULT Lock(bool readOnly = true)
       {
          if (m_rect.pBits != NULL) return S_OK;
          DWORD flags = readOnly ? D3DLOCK_READONLY : 0;
          return m_surface->LockRect(&m_rect, NULL, flags);
       }
    ...
    vector<OffscreenInfo>                    m_offscreenInfo;
    int                                      m_offscreenIndex;
};
